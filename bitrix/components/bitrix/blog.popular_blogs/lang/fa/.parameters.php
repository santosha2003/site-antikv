<?
$MESS["BLOG_DESCR_SORT_ASC"] = "Ascending";
$MESS["BMN_BLOG_VAR"] = "Blog identifier variable";
$MESS["BMN_USER_VAR"] = "Blog identifier variable";
$MESS["BLOG_DESCR_BLOG_NAME"] = "Blog name";
$MESS["BMN_PATH_TO_BLOG"] = "Blog page path template";
$MESS["BLG_GROUP_ID"] = "Blogs group";
$MESS["BLOG_DESCR_DATE_CREATE"] = "Create date";
$MESS["BLOG_DESCR_LAST_MES"] = "Date of last post";
$MESS["BLOG_DESCR_SORT_DESC"] = "Descending";
$MESS["BLOG_DESCR_SORT_ORDER"] = "Direction for the first sorting pass";
$MESS["BLOG_DESCR_SORT_1"] = "Field for first sorting pass";
$MESS["BLOG_DESCR_SORT_2"] = "Field for second sorting pass";
$MESS["BMN_PATH_TO_GROUP_BLOG"] = "Group Blog Page URL Template";
$MESS["BMN_PATH_TO_GROUP"] = "Group Profile Page URL Template";
$MESS["BLOG_DESCR_N"] = "خیر";
$MESS["BLOG_DESCR_NB_COUNT"] = "Number of blogs to show";
$MESS["BMN_PAGE_VAR"] = "Page variable";
$MESS["BLOG_DESCR_NB_S_DESCR"] = "Show blog description";
$MESS["BMNP_PERIOD_DAYS"] = "Show recent popular blog posts for (days)";
$MESS["BMN_PATH_TO_USER"] = "Template of the user profile page path";
$MESS["B_VARIABLE_ALIASES"] = "Variable aliases";
$MESS["BLOG_DESCR_Y"] = "بله";
?>