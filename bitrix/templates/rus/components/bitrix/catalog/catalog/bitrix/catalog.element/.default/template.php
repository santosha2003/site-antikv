...
<div class="tovar">		
									<div class="prodItem"><div class="inbg">
										<div class="prPrice"><?=$arResult["PRICES"]["BASE"]["VALUE"]?> <br /><?=$arResult["PRICES"]["BASE"]["CURRENCY"] == "RUB" ? "руб." : $arResult["PRICES"]["BASE"]["CURRENCY"]?></div>
...
<?if ($arResult["DISPLAY_PROPERTIES"]["TIRAGE"]):?><p><b>Тираж выпуска:</b> <?=$arResult["DISPLAY_PROPERTIES"]["TIRAGE"]["VALUE"]?> </p><?endif?>
<?if ($arResult["DISPLAY_PROPERTIES"]["CONDITIO"]):?><p>Состояние: <?=$arResult["DISPLAY_PROPERTIES"]["CONDITIO"]["VALUE"]?> </p><?endif?>
...
?>
  ...
  <div class="bigFOTO"><table><tr><td><a href="<?=$spk_bigImageSrc?>" class="fancy">
<img src="<?=$renderImage2['src']?>" height="<?=$newHeight2?>" alt="<?=$arResult["NAME"]?>" />
<?//echo CFile::ShowImage($renderImage2['src'], $newWidth2, $newHeight2, "border=0", "", true);?></a></td></tr></table></div>
  ...
  <ul id="mycarousel" class="jcarousel jcarousel-skin-tango">
 						<?if (count($arResult["DISPLAY_PROPERTIES"]["FOTO"]["VALUE"])>1):?>
								<?foreach($arResult["DISPLAY_PROPERTIES"]["FOTO"]["FILE_VALUE"] as $pid=>$arProperty):?>
	                            	...
	                            	?>
									<li rel="<?=$arProperty["SRC"]?>"><div class="fotosl"><table><tr><td<?if ($pid==0):?> class="active"<?endif?>><?echo CFile::ShowImage($renderImage['src'], $newWidth, $newHeight, "border=0", "", true);?></td></tr></table></div><span<?if ($pid==0):?> class="active"<?endif?>></span></li>
								<?endforeach?>
							<?else:?>
									<?  $newWidth=115;
	                            		$newHeight=115;
	                            	...
	                            	?>
									<li rel="<?echo $arResult["DISPLAY_PROPERTIES"]["FOTO"]["FILE_VALUE"]["SRC"];?>"><div class="fotosl"><table><tr><td<?if ($pid==0):?> class="active"<?endif?>><?echo CFile::ShowImage($renderImage['src'], $newWidth, $newHeight, "border=0", "", true);?></td></tr></table></div><span class="active"></span></li>
							<?endif?>        
