...
<?php
			...
			{$id=$arProperty['VALUE'];
             $link=new mysqli("localhost:3306","p90","cumine3N","newgame_bx");
            // $link=mysqli_connect("localhost:3306","p90","cumine3N");  php5 --> 7.2
             if (!$link) { echo ('login in templ catalog element check! Could not connect: ' . mysqli_error());  //or "login in templ catalog element check! Could not connect:" . mysql_error();
             }
        
// mysqli_select_db("newgame_bx");
 $result1=$link->query("SELECT PREVIEW_PICTURE FROM `b_iblock_element` WHERE  (`ID`='$id') LIMIT 1"); // or die(mysql_error());
     if (!$result1) { echo('err: ' . mysqli_error()); }
   for ($data9=array(); $row=mysqli_fetch_row($result1);$data9[]=$row);
           $idphoto=$data9[0][0];
   $result1=$link->query("SELECT `SUBDIR`, `FILE_NAME` FROM `b_file` WHERE  (`ID`='$idphoto') LIMIT 1");// or (mysql_error();  
    if (!$result1) { echo('err: ' . mysqli_error()); }
   for ($data9=array(); $row=mysqli_fetch_row($result1);$data9[]=$row);
...
?>
	...
	<?php }
         ...
         $id=$arProperty['VALUE'];
             $link=new mysqli("localhost:3306","p90","cumine3N","newgame_bx");
             if (!$link) { echo ('login in templ catalog element check! Could not connect: ' . mysql_error());  //or "login in templ catalog element check! Could not connect:" . mysql_error();
             }
// mysql_select_db("newgame_bx");   php v7.2 removed - unbuff. query now
 $result1=$link->query("SELECT `PREVIEW_PICTURE`, `NAME` FROM `b_iblock_element` WHERE  (`ID`='$id') LIMIT 1"); // or die(mysql_error());  
     if (!$result1) { echo('err: ' . mysqli_error()); }
   for ($data9=array(); $row=mysqli_fetch_row($result1);$data9[]=$row);
           $idphoto=$data9[0][0];
                 $str=$data9[0][1];
   $result1=$link->query("SELECT `SUBDIR`, `FILE_NAME` FROM `b_file` WHERE  (`ID`='$idphoto') LIMIT 1"); // or die(mysql_error());  
    if (!$result1) { echo('err: ' . mysqli_error()); }
   for ($data9=array(); $row=mysqli_fetch_row($result1);$data9[]=$row);
