...
<?php
			...
			{$id=$arProperty['VALUE'];
             $link=mysqli_connect("localhost:3306","p90","cumine3N");
 ...
 mysqli_select_db("shedevr_ru");
 $result1=mysqli_unbuffered_query("SELECT PREVIEW_PICTURE FROM `b_iblock_element` WHERE  (`ID`='$id') LIMIT 1"); // or die(mysql_error());
     if (!$result1) { echo('err: ' . mysqli_error()); }
   for ($data9=array(); $row=mysqli_fetch_row($result1);$data9[]=$row);
           $idphoto=$data9['0']['0'];
   $result1=mysqli_unbuffered_query("SELECT `SUBDIR`, `FILE_NAME` FROM `b_file` WHERE  (`ID`='$idphoto') LIMIT 1");// or (mysql_error();  
    if (!$result1) { echo('err: ' . mysqli_error()); }
   for ($data9=array(); $row=mysqli_fetch_row($result1);$data9[]=$row);
...
?>
 ...
 mysqli_select_db("shedevr_ru");
 $result1=mysqli_unbuffered_query("SELECT `PREVIEW_PICTURE`, `NAME` FROM `b_iblock_element` WHERE  (`ID`='$id') LIMIT 1"); // or die(mysql_error());  
     if (!$result1) { echo('err: ' . mysqli_error()); }
   for ($data9=array(); $row=mysqli_fetch_row($result1);$data9[]=$row);
           $idphoto=$data9['0']['0'];
                 $str=$data9['0']['1'];
   $result1=mysqli_unbuffered_query("SELECT `SUBDIR`, `FILE_NAME` FROM `b_file` WHERE  (`ID`='$idphoto') LIMIT 1"); // or die(mysql_error());  
    if (!$result1) { echo('err: ' . mysqli_error()); }
   for ($data9=array(); $row=mysqli_fetch_row($result1);$data9[]=$row);
