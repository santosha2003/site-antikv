	...
	implements IBPEventActivity, IBPActivityExternalEventListener
{
	...
	public static function GetPropertiesDialog($documentType, $activityName, $arWorkflowTemplate, $arWorkflowParameters, $arWorkflowVariables, $currentValues = null, $formName = "")
	{
		...
		{
			$currentValues['template'] = self::extractTemplateParameterValues($currentValues['template_id'], $currentValues);
		}
		...
		if ($currentTemplateId)
		{
			...
			$templateParametersRender = self::renderTemplateParametersForm($currentTemplateId, $formName, $currentValues['template']);
		}
	...
	public static function GetPropertiesDialogValues($documentType, $activityName, &$workflowTemplate, &$arWorkflowParameters, &$arWorkflowVariables, $currentValues, &$errors)
	{
		...
		$properties = array(
			...
			'TemplateParameters' => self::extractTemplateParameterValues($currentValues['template_id'], $currentValues, $errors),
		);
	...
	public static function getAjaxResponse($request)
	{
		...
		{
			$result = self::renderTemplateParametersForm($request['template_id'], $request['form_name']);
		}
	...
	private static function extractTemplateParameterValues($templateId, $request, &$errors = array())
	{
	...
	private static function renderTemplateParametersForm($templateId, $formName, array $currentValues = array())
	{
		$result = '';
		$template = self::getTemplate($templateId);
		if ($template)
