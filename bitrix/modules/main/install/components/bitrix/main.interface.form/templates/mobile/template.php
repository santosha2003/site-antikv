			...
			<div style="height: 100%;">
				<div class="bx-edit-table <?=(isset($tab["class"]) ? $tab['class'] : '')?>" id="<?=$tab["id"]?>_edit_table"><?
					foreach($tab["fields"] as $field)
					{
						...
						if ($arParams["RESTRICTED_MODE"] && empty($field["value"]))
							continue;
						...
						else
						{
							...
							switch($field["type"])
							{
								...
								case 'select-user':
									...
									$users = ob_get_clean();
									$html = "<select name=\"{$field["id"]}\" bx-type=\"select-user\" bx-can-drop=\"".($field["canDrop"] === false ? "false" : "")."\" id=\"{$field["~id"]}\"{$params}\">".$html."</select>".
										"<span id=\"{$field["~id"]}_target\">".$users."</span>".
								...
								case 'select-users':
									...
									$users = ob_get_clean();
									$html = "<select name=\"{$field["id"]}\" bx-type=\"select-user\" bx-can-drop=\"".($field["canDrop"] === false ? "false" : "")."\" id=\"{$field["~id"]}\" multiple {$params}\">".$html."</select>".
										"<span id=\"{$field["~id"]}_target\">".$users."</span>".
								...
								case 'disk':
									...
									$APPLICATION->IncludeComponent("bitrix:system.field.edit", "disk_file",
										array("arUserField" => $val, "MOBILE" => "Y"),
										$component,
										array("HIDE_ICONS" => "Y")
									);
									$html = ob_get_clean();
...
?>
	...
	<?if($arParams["~BUTTONS"]["standard_buttons"] !== false):?>
		...
		<a href="#" onclick="app.closeController({ drop: true });"><?=GetMessage("interface_form_cancel")?></a>
	<?endif?>
