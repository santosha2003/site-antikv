;(function() {
				...
				callback : function(data) {
					...
					if (data && data.values && data.values.length > 0)
					{
						...
						if (html === '' && !this.multiple)
							html = '<span style="color:grey">' + GetMessage("interface_form_select") + '</span>';
		...
		var d = function(select, eventNode, container) {
			...
			d.prototype = {
				...
				callback : function(data) {
					...
					for (ii = 0; ii < Math.min((this.multiple ? data.a_users.length : 1), data.a_users.length); ii++)
					{
						user = data.a_users[ii];
						...
						html += ([
							'<div class="mobile-grid-field-select-user-item">',
								...
								'<span>' + BX.util.htmlspecialchars(user["NAME"]) + '</span>',
							'</div>'
	...
	BX.Mobile.Grid.Form = function(params) {
		...
		if (typeof params === "object")
		{
			...
			{
				this.click = BX.delegate(this.click, this);
				BX.bind(BX('submit_' + this.formId), "click", this.click);
			}
	...
	BX.Mobile.Grid.Form.prototype = {
		...
		bindElement : function(node) {
			...
			if (BX(node))
			{
				...
				else if (tag == 'textarea' || node.getAttribute("type") == "text")
				{
