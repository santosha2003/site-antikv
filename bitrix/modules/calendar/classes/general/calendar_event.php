...
//					}
	...
	public static function ParseRecursion(&$res, $event, $Params = array())
	{
		...
		if ($limitToTS > $event['DATE_TO_TS_UTC'] + CCalendar::GetDayLen())
			$limitToTS = $event['DATE_TO_TS_UTC'] + CCalendar::GetDayLen();
	...
	public static function CreateChildEvents($parentId, $arFields, $Params, $userId = 0)
	{
		...
		$arAttendees = $arFields['ATTENDEES'];
				...
				foreach($arAttendees as $userKey)
				{
					...
					if ($attendeeId && (intVal($arFields['CREATED_BY']) !== $attendeeId || $arFields['CAL_TYPE'] != 'user'))
					{
						...
						if (!$bNew && $curAttendeesIndex[$attendeeId])
						{
							...
							unset($childParams['arFields']['COLOR'], $childParams['arFields']['TEXT_COLOR'], $childParams['arFields']['REMIND']);
...
//	public static function InviteAttendeesOld($ID, $arFields = array(), $arAttendees = array(), $arStatuses = array(), $bSendInvitations = true, $userId = 0)
//	{
//		return;
//		global $DB, $CACHE_MANAGER;
//		$ID = intVal($ID);
...
//	}
	...
	public static function GetAccessibilityForUsers($params = array())
	{
		...
		foreach($events as $event)
		{
			...
			$accessibility[$event['CREATED_BY']][] = array(
				"ID" => $event["ID"],
				"NAME" => $event["NAME"],
