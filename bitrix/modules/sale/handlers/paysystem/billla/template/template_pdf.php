...
{
	$pdf->SetX($pdf->GetX() + $logoWidth);
	$pdf->MultiCell(0, 15, CSalePdf::prepareToPdf($params["SELLER_COMPANY_ADDRESS"]), 0, 'L');
}

if ($params["SELLER_COMPANY_PHONE"])
{
	$pdf->SetX($pdf->GetX() + $logoWidth);
	$pdf->Write(15, CSalePdf::prepareToPdf(sprintf(Loc::getMessage('SALE_HPS_BILLLA_COMPANY_PHONE').".: %s", $params["SELLER_COMPANY_PHONE"])));
	$pdf->Ln();
}
...
$pdf->Cell(0, 15, CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLLA_TITLE')), 0, 0, 'C');
...
$invoiceNo = CSalePdf::prepareToPdf($payment->getField("ACCOUNT_NUMBER"));
...
$title = CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLLA_TITLE').' # ');
...
if ($params["BUYER_PERSON_COMPANY_NAME"])
{
	...
	{
		$pdf->Write(15, CSalePdf::prepareToPdf($params["BUYER_PERSON_COMPANY_ADDRESS"]));
	}
}
...
if (count($basket->getBasketItems()) > 0)
{
	$arColsCaption = array(
		...
		CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLLA_BASKET_ITEM')),
		CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLLA_BASKET_ITEM_QUANTITY')),
		CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLLA_BASKET_ITEM_MEASURE')),
		CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLLA_BASKET_ITEM_PRICE')),
		CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLLA_BASKET_ITEM_TAX')),
		CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLLA_BASKET_ITEM_TOTAL'))
	);
	$arCells = array();
	$arProps = array();
	...
	foreach ($basket->getBasketItems() as $basketItem)
	{
		...
		$arCells[++$n] = array(
			...
			CSalePdf::prepareToPdf($basketItem->getField("MEASURE_NAME") ? $basketItem->getField("MEASURE_NAME") : Loc::getMessage('SALE_HPS_BILLLA_MEASURE')),
	...
	if ($sum < $order->getPrice())
	{
		$arCells[++$n] = array(
			...
			null,
			null,
			CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLLA_SUB_TOTAL').":"),
	...
	if (DoubleVal($sumPaid) > 0)
	{
		$arCells[++$n] = array(
			...
			null,
			null,
			CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLLA_SUM_PAID').":"),
			CSalePdf::prepareToPdf(SaleFormatCurrency(
				$sumPaid,
				$order->getCurrency(),
				false
	...
	if (DoubleVal($order->getDiscountPrice()) > 0)
	{
		$arCells[++$n] = array(
			...
			null,
			null,
			CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLLA_DISCOUNT').":"),
			CSalePdf::prepareToPdf(SaleFormatCurrency(
	...
	$arCells[++$n] = array(
		...
		null,
		null,
		CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLLA_TOTAL').":"),
		CSalePdf::prepareToPdf(SaleFormatCurrency(
			$payment->getSum(),
			$order->getCurrency(),
			false
...
if ($params['BILLLA_PATH_TO_STAMP'])
{
	...
	if (!$blank && \Bitrix\Main\IO\File::isFileExists($filePath))
	{
...
$bankAccNo = $params["SELLER_COMPANY_BANK_ACCNO"];
$bankRouteNo = $params["SELLER_COMPANY_BANK_ROUTENO"];
...
if ($bankAccNo && $bankRouteNo && $bankSwift)
{
	...
	$bank = $params["SELLER_COMPANY_BANK"];
