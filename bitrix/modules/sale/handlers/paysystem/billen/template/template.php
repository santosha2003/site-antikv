...
if ($params['BACKGROUND'])
{
	...
	$backgroundStyle = $params['BACKGROUND_STYLE'];
...
$margin = array(
	'top' => intval($params['MARGIN_TOP'] ?: 15) * 72/25.4,
	'right' => intval($params['MARGIN_RIGHT'] ?: 15) * 72/25.4,
	'bottom' => intval($params['MARGIN_BOTTOM'] ?: 15) * 72/25.4,
	'left' => intval($params['MARGIN_LEFT'] ?: 20) * 72/25.4
);
...
<table class="header">
	<tr>
		<? if ($params["PATH_TO_LOGO"]) { ?>
		<td style="padding-right: 5pt; ">
			<? $imgParams = CFile::_GetImgParams($params['PATH_TO_LOGO']); ?>
			<? $imgWidth = $imgParams['WIDTH'] * 96 / (intval($params['LOGO_DPI']) ?: 96); ?>
		...
		<td>
			...
			<b><?=$params["SELLER_COMPANY_ADDRESS"]; ?></b><br>
			<? } ?>
...
<table width="100%">
	<tr>
		...
		<td>
			...
			<?=$params["BUYER_PERSON_COMPANY_ADDRESS"]; ?>
			<? } ?>
		</td>
		<? } ?>
		<td align="right">
			<table class="inv">
				<tr align="right">
					<td><b>Invoice #&nbsp;</b></td>
					<td><?=htmlspecialcharsbx($payment->getField('ACCOUNT_NUMBER')); ?></td>
				</tr>
				<tr align="right">
...
if (count($basket->getBasketItems()) > 0)
{
	...
	foreach ($basket->getBasketItems() as $basketItem)
	{
		...
		if ($basketItem->isVatInPrice())
			$vatLessBasket = roundEx($basketItem->getPrice() / (1 + $basketItem->getVatRate()), SALE_VALUE_PRECISION);
		else
			$vatLessBasket = $basketItem->getPrice();
		...
		$arCells[++$n] = array(
			...
			SaleFormatCurrency($vatLessBasket, $basketItem->getCurrency(), false),
			roundEx($basketItem->getVatRate()*100, SALE_VALUE_PRECISION) . "%",
			SaleFormatCurrency(
				$vatLessBasket * $basketItem->getQuantity(),
		...
		$sum += doubleval($vatLessBasket * $basketItem->getQuantity());
		...
		if ($basketItem->getVatRate() > 0)
		{
			...
			$vats[$basketItem->getVatRate()] += ($basketItem->getPrice() - $vatLessBasket) * $basketItem->getQuantity();
		}
	}
	...
	if (!empty($vats))
	{
		...
		$delta = intval(roundEx(
			$order->getPrice() - $sum - array_sum($vats),
			SALE_VALUE_PRECISION
...
<? if ($params["COMMENT1"] || $params["COMMENT2"]) { ?>
<b>Terms & Conditions</b>
<br>
	<? if ($params["COMMENT1"]) { ?>
	<?=nl2br(HTMLToTxt(preg_replace(
		array('#</div>\s*<div[^>]*>#i', '#</?div>#i'), array('<br>', '<br>'),
		htmlspecialcharsback($params["COMMENT1"])
	), '', array(), 0)); ?>
	<br>
	<br>
	<? } ?>
	...
	<?=nl2br(HTMLToTxt(preg_replace(
		array('#</div>\s*<div[^>]*>#i', '#</?div>#i'), array('<br>', '<br>'),
		htmlspecialcharsback($params["COMMENT2"])
	), '', array(), 0)); ?>
	<br>
	<br>
	<? } ?>
