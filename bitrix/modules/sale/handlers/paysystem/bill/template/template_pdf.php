...
if ($params["SELLER_COMPANY_BANK_NAME"])
{
	$sellerBank = sprintf(
		...
		$params["SELLER_COMPANY_BANK_CITY"]
	);
...
$pdf->Cell(0, 18, CSalePdf::prepareToPdf($params["SELLER_COMPANY_BANK_BIK"]));
...
$pdf->Cell(0, 18, CSalePdf::prepareToPdf($params["SELLER_COMPANY_BANK_ACCOUNT_CORR"]));
...
$billNo_tmp = CSalePdf::prepareToPdf(
	Loc::getMessage('SALE_HPS_BILL_SELLER_TITLE', array('#PAYMENT_NUM#' => $payment->getField("ACCOUNT_NUMBER"), '#PAYMENT_DATE#' => $params["PAYMENT_DATE_INSERT"]))
);
...
if ($params["BUYER_PERSON_COMPANY_NAME"])
{
	...
	if ($params["BUYER_PERSON_COMPANY_INN"])
		$pdf->Write(15, CSalePdf::prepareToPdf(Loc::getMessage('BUYER_PERSON_COMPANY_INN', array('#INN#' => $params["BUYER_PERSON_COMPANY_INN"]))));
	if ($params["BUYER_PERSON_COMPANY_ADDRESS"])
		$pdf->Write(15, CSalePdf::prepareToPdf(sprintf(", %s", $params["BUYER_PERSON_COMPANY_ADDRESS"])));
