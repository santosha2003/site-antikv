...
<table class="acc" width="100%">
	...
	<tr>
		...
		<td>
			<?=$params["SELLER_COMPANY_BANK_BIK"]; ?><br>
...
<table width="100%">
	...
	<tr>
		...
		<td style="font-size: 2em; font-weight: bold; text-align: center">
			<nobr><?=Loc::getMessage('SALE_HPS_BILL_SELLER_TITLE', array('#PAYMENT_NUM#' => $payment->getField("ACCOUNT_NUMBER"), '#PAYMENT_DATE#' => $params["PAYMENT_DATE_INSERT"]));?>
			</nobr>
		</td>
		<td></td>
	</tr>
...
if ($params["BUYER_PERSON_COMPANY_NAME"]) {
	...
	if ($params["BUYER_PERSON_COMPANY_INN"])
		echo Loc::getMessage('BUYER_PERSON_COMPANY_INN', array('#INN#' => $params["BUYER_PERSON_COMPANY_INN"]));
	if ($params["BUYER_PERSON_COMPANY_ADDRESS"])
		echo sprintf(", %s", $params["BUYER_PERSON_COMPANY_ADDRESS"]);
...
foreach ($basketItems as $basketItem)
{
	...
	$cells[++$n] = array(
			...
			SaleFormatCurrency(
					$basketItem->getField("PRICE") * $basketItem->getQuantity(),
