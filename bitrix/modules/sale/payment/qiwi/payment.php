...
$paymentId 	= trim(CSalePaySystemAction::GetParamValue("ORDER_PAYMENT_ID"));
...
<?if(!preg_match($validPhone, $to)):?>
	<form  action="<?= POST_FORM_ACTION_URI?>" method="post">
		...
		<p><?=htmlspecialcharsbx(Loc::getMessage("SALE_QH_INPUT_PHONE"))?></p>
...
<?else:?>
	<form  action="<?=$createUrl?>" method="post">
		...
		<input type="hidden" name="txn_id" value="<?=$txnId.':'.$paymentId;?>"/>
