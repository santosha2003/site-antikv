...
class CAllSalePaySystemAction
{
	...
	function InitParamArrays($arOrder, $orderID = 0, $psParams = "", $relatedData = array(), $payment = array())
	{
		...
		if ($paySystemId !== '')
		{
			$params = CSalePaySystemAction::getParamsByConsumer('PAYSYSTEM_'.$paySystemId, $arOrder['PERSON_TYPE_ID']);
		...
		while ($paySystem = $dbRes->fetch())
		{
			...
			if (isset($oldActionFiles[$paySystem['ACTION_FILE']]) && !IsModuleInstalled('intranet'))
			{
				...
				if (isset($params['PAYMENT_DATE_INSERT']))
					$params['PAYMENT_DATE_INSERT'] = array('TYPE' => 'PAYMENT', 'VALUE' => 'DATE_BILL');
	...
	public static function getCodesAliases($handler)
	{
		$psAliases = array(
			...
			'bill' => array(
				...
				'SELLER_BIK' => 'SELLER_COMPANY_BANK_BIK',
