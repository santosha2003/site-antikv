...
class CSaleDiscountActionApply
{
	...
	public static function ApplyGiftDiscount(&$order, $callableFilter)
	{
		...
		else
		{
			...
			$discountBasket = array_filter($itemsCopy, $callableFilter);
		}
		if (empty($discountBasket))
			return;
		...
		$discountDescr = Loc::getMessage('BT_SALE_GIFT_DISCOUNT_DESCR');
		if (!$manualMode)
		{
			$prepareResult = Sale\OrderDiscountManager::prepareDiscountDescription(Sale\OrderDiscountManager::DESCR_TYPE_SIMPLE, $discountDescr);
			if ($prepareResult->isSuccess())
			{
				if (!isset($order['DISCOUNT_DESCR']))
					$order['DISCOUNT_DESCR'] = array();
		...
		$applyResultList = array();
		\Bitrix\Main\Type\Collection::sortByColumn($clearBasket, 'PRICE', null, null, true);
		foreach ($clearBasket as $basketCode => $arOneRow)
		{
			$arOneRow['DISCOUNT_PRICE'] = $arOneRow['BASE_PRICE'];
			$arOneRow['PRICE'] = 0;
		...
		foreach ($applyResultList as $basketCode => $applyResult)
		{
			$prepareResult = Sale\OrderDiscountManager::prepareDiscountDescription(Sale\OrderDiscountManager::DESCR_TYPE_SIMPLE, $applyResult);
			if ($prepareResult->isSuccess())
			{
				if (!isset($order['DISCOUNT_RESULT']['BASKET'][$basketCode]))
					$order['DISCOUNT_RESULT']['BASKET'][$basketCode] = array();
...
class CSaleActionGiftCtrlGroup extends CSaleActionCtrlGroup
{
	...
	public static function Generate($arOneCondition, $arParams, $arControl, $arSubs = false)
	{
		...
		{
			$mxResult = 'CSaleDiscountActionApply::ApplyGiftDiscount(' . $arParams['ORDER'] . ', ' . implode('; ',$arSubs) . ');';
		}
		return $mxResult;
	}
