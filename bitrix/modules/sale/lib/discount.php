...
class Discount
{
	...
	public function setBasketItemBasePrice($code, $price, $currency)
	{
		...
		$this->basketBasePrice[$code] = (
			...
			? $price
			: roundEx(\CCurrencyRates::convertCurrency($price, $currency, $orderCurrency), SALE_VALUE_PRECISION)
		);
	...
	public function setBasketBasePrice($basket)
	{
		...
		foreach ($basket as $code => $basketItem)
		{
			$this->basketBasePrice[$code] = (
				...
				? $basketItem['PRICE']
				: roundEx(\CCurrencyRates::convertCurrency($basketItem['PRICE'], $basketItem['CURRENCY'], $orderCurrency), SALE_VALUE_PRECISION)
			);
		}
...
/*			if ($orderMode == Order::SALE_ORDER_CALC_TYPE_REFRESH)
		...
		/*
		 * Discount::fillEmptyOrderData used in $this->loadOrderData();
	...
	protected function loadOrderData()
	{
		...
		$skipLoadData = (!$this->isOrderNew() && ($this->getUseMode() == self::USE_MODE_FULL && !$this->isMixedBasket()));
		...
		$this->convertedOrder = !$this->isOrderNew();
		if (!$this->isOrderNew())
		{
			if (!$skipLoadData)
			{
