...
class Manager
{
	...
	public static function getRestrictedObjectsList(Shipment $shipment, $restrictionMode = Restrictions\Manager::MODE_CLIENT)
	{
		...
		foreach($services as $srvParams)
		{
			...
			$res = Restrictions\Manager::checkService($srvParams["ID"], $shipment, $restrictionMode);

			if($res == Restrictions\Manager::SEVERITY_STRICT)
				continue;

			if($res == Restrictions\Manager::SEVERITY_SOFT)
	 ...
	 * @throws \Bitrix\Main\ArgumentException
	 */
	public static function getActiveList($calculatingOnly = false)
	{
		...
		if(!$isDataPrepared)
		{
			...
			self::$cachedFields = $result;
			Restrictions\Manager::prepareData(array_keys(self::$cachedFields));
			ExtraServices\Manager::prepareData(array_keys(self::$cachedFields));
			$isDataPrepared = true;
		}
		else
		{
			$result = array_intersect_key(self::$cachedFields, $activeIds);
		}
		...
		if (is_array($resultList) && !empty($resultList))
		{
			...
			foreach ($resultList as $eventResult)
			{
				...
				if ($eventResult->getType() != EventResult::SUCCESS)
					throw new SystemException("Can't add custom delivery service class successfully");
	...
	public static function checkServiceRestriction($deliveryId, $checkParams, $restrictionClassName)
	{
		...
		return $restriction->check($checkParams, $restrictionFields["PARAMS"], $deliveryId);
	}
