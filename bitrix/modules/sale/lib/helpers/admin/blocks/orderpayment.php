...
class OrderPayment
{
	...
	private static function getEditTemplate($data, $index, $post = array())
	{
		...
		if ($index == 1)
			$hiddenPaySystemInnerId = '<input type="hidden" value="'.\Bitrix\Sale\PaySystem\Manager::getInnerPaySystemId().'" id="PAYMENT_INNER_BUDGET_ID">';
	...
	public static function getPaySystemList(Payment $payment)
	{
		...
		$paySystems = Manager::getListWithRestrictions($payment, Restrictions\Manager::MODE_MANAGER);
	...
	public static function updateData(Order &$order, $payments, $canSetPaid = false)
	{
		...
		foreach ($payments as $payment)
		{
			...
			$psService = PaySystemActionTable::getRow(
				array(
					'select' => array('NAME'),
					'filter' => array('ID' => $payment['PAY_SYSTEM_ID'])
				)
			);
			...
			$paymentFields = array(
				...
				'PAY_SYSTEM_NAME' => ($psService) ? $psService['NAME'] : ''
			);
