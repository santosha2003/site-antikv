...
class OrderShipment
{
	...
	protected static function prepareData($error = false, $needRecalculate = true)
	{
		...
		$fields['HAS_TRACKING'] = !is_null($delivery) && strlen($delivery->getTrackingClass()) > 0 ? true : false;
	...
	public static function updateData(Order &$order, array $shipments)
	{
		...
		foreach ($shipments as $item)
		{
			...
			try
			{
				if($service = Services\Manager::getObjectById($shipmentFields['DELIVERY_ID']))
				{
					if ($service->getParentService())
