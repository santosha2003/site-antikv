...
if($this->startResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()))))
{
	...
	foreach ($arResult["SECTIONS"] as &$arSection)
	{
		...
		if ($arParams["COUNT_ELEMENTS"])
		{
			...
			{
                $result['INCLUDE_SUBSECTIONS'] = 'N';
			}

            if ($arSection['IBLOCK_ID'] == 1) {
                debugLog($arSection['IBLOCK_ID'], 'khjkjhk.log');
                $elementFilter['IBLOCK_SECTION_ID'] = $elementFilter['SECTION_ID'];
                unset($elementFilter['SECTION_ID']);
            }
