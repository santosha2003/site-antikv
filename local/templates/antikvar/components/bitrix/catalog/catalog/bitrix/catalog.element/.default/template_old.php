    ...
    <div class="product-info">
        <div class="top">
            <div class="product-imgs">
                <pre style="display: none"><?print_r($arParams["PROPERTY_CODE"])?></pre>
        <? if (count($arResult["DISPLAY_PROPERTIES"]["FOTO"]["VALUE"]) > 0): ?>
            <div class="slaider">
                ...
                */?>
                ...
                <div class="bigFOTO">
					<?php if(is_array($arResult["DISPLAY_PROPERTIES"]["FOTO"]["FILE_VALUE"][0])): ?>
						<? foreach ($arResult["DISPLAY_PROPERTIES"]["FOTO"]["FILE_VALUE"] as $pid => $arProperty):?>
							<?
                                ...
                                // Big thumb with watermark
								if ($bigThumbUse) {
                                    $renderImage2 = CFile::ResizeImageGet($arProperty['ID'], $bigThumbSizes, $bigThumbMethod, false, $bigThumbWatermark);
								...
								if ($origUse) {
                                    ...
                                    $renderWater2 = CFile::ResizeImageGet($arProperty['ID'], $origImgSizes, $origImgMethod, false, $origImgWatermark);
					...
					<?php else: ?>
							<?
								//$renderImage2 = CFile::ResizeImageGet($arResult["DISPLAY_PROPERTIES"]["FOTO"]["FILE_VALUE"]['ID'], Array("width" => $newWidth2, "height" => $newHeight2), "BX_RESIZE_IMAGE_EXACT");

                                // Big thumb with watermark
                                if ($bigThumbUse) {
                                    $renderImage2 = CFile::ResizeImageGet($arResult["DISPLAY_PROPERTIES"]["FOTO"]["FILE_VALUE"]['ID'], $bigThumbSizes, $bigThumbMethod, false, $bigThumbWatermark);
                                } else {
                                    $renderImage2 = CFile::ResizeImageGet($arResult["DISPLAY_PROPERTIES"]["FOTO"]["FILE_VALUE"]['ID'], Array("width" => $newWidth2, "height" => $newHeight2), "BX_RESIZE_IMAGE_EXACT");
                                }
                                ...
                                // Original without watermark
                                //$sImgSrc = $arResult["DISPLAY_PROPERTIES"]["FOTO"]["FILE_VALUE"]['SRC'];

                                if ($origUse) {
                                    // Original with watermark
                                    $renderWater2 = CFile::ResizeImageGet($arResult["DISPLAY_PROPERTIES"]["FOTO"]["FILE_VALUE"]['ID'], $origImgSizes, $origImgMethod, false, $origImgWatermark);
        ...
        <? endif ?>
            ...
            <div class="left">
                <div class="property">
                    <? if ($arResult["DISPLAY_PROPERTIES"]["YEAR"]): ?><p><b>Изготовитель, год:</b> <?= $arResult["DISPLAY_PROPERTIES"]["YEAR"]["VALUE"] ?></p> <? endif ?>
                    <? if ($arResult["DISPLAY_PROPERTIES"]["MATERIAL"]): ?><p>
                        <b>Материал, вес:</b> <?= $arResult["DISPLAY_PROPERTIES"]["MATERIAL"]["VALUE"] ?></p> <? endif ?>
                    <? if ($arResult["DISPLAY_PROPERTIES"]["DIAMETR"]): ?><p>
                        <b>Размеры:</b> <?= $arResult["DISPLAY_PROPERTIES"]["DIAMETR"]["VALUE"] ?> </p><? endif ?>
                    ...
                    <? if ($arResult["DISPLAY_PROPERTIES"]["CONDITIO"]): ?><p><b>
                        Сохранность:</b> <?= $arResult["DISPLAY_PROPERTIES"]["CONDITIO"]["VALUE"] ?> </p><? endif ?>

                    <? if ($arResult["DISPLAY_PROPERTIES"]["RARITY"]): ?><p><b>
                        Редкость:</b> <?= $arResult["DISPLAY_PROPERTIES"]["RARITY"]["VALUE"] ?> </p><? endif ?>
                </div>
            </div>
            <div class="right">
                <div class="price">
                    ...
                    $hidePriceText = isset($arResult['DISPLAY_PROPERTIES']['HIDE_PRICE']['DISPLAY_VALUE']) ? $arResult['DISPLAY_PROPERTIES']['HIDE_PRICE']['DISPLAY_VALUE'] : '';
                    ?>
